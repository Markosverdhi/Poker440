...
def evaluate_hand(cards):
    ...
    if not straight and set([14, 2, 3, 4, 5]).issubset(set(ranks)):
        straight = True
        straight_high = 5
...
class BaseFullPokerEnv:
    ...
    def step(self, action: str):
        ...
        # Provide a small intermediate step penalty reward for shaping.
        return self._get_obs(self.agent_id), -0.1, False, {}
    ...
    def _process_action(self, player: int, action: str) -> None:
        ...
        if action == 'fold':
            ...
            # Dynamically compute a small bet.
            dynamic_raise = max(50, int(0.1 * self.stacks[player]))
            raise_amount = dynamic_raise
            ...
            # Dynamically compute a big bet.
            dynamic_raise = max(100, int(0.2 * self.stacks[player]))
            raise_amount = dynamic_raise
            new_bet = self.current_max_bet + raise_amount
            amount_to_call = new_bet - self.current_bets[player]
            amount = min(amount_to_call, self.stacks[player])
