...
class PokerApp:
    ...
    def _start_new_round(self):
        ...
        print(f"Dealer: {self.env.dealer}, SB Position: {self.env.small_blind_pos}, BB Position: {self.env.big_blind_pos}")
    ...
    def _handle_round_end(self):
        ...
        # Store players active right before finalization (might include winner by default)
        self.last_round_active_players = self.env.active_players[:]
        print(f"Debug Round End: Stored last_round_active_players = {[p+1 for p in self.last_round_active_players]}")

        # Disable action buttons and update turn label
        for btn in self.action_buttons.values():
            btn.config(state=tk.DISABLED)
        self.turn_label.config(text="Turn: Round Over", foreground="darkred") # Change text

        # Ensure board is complete for evaluation if needed (mainly for display)
        ...
        win_amount = 0
        scores = {}
        # Use temporary stack copy for display in message box
        final_stacks_display = self.env.stacks.copy()
        pot_to_distribute = self.env.pot
        eligible_players = self.last_round_active_players
                ...
                for pid, score_tuple in final_scores.items():
                    if isinstance(score_tuple, tuple):
                        if best_score_tuple is None or score_tuple > best_score_tuple:
                            ...
                            if pid not in winners: winners.append(pid)

            # Calculate winnings for display
            if winners:
                win_amount = pot_to_distribute / len(winners)
                for pid in winners:
                    final_stacks_display[pid] = final_stacks_display.get(pid, 0) + win_amount
        else:
            print("Warning: Round ended with no eligible players.")
        # --- End Winner Calculation ---


        # --- CARD DISPLAY LOGIC ---
        print(f"Debug Showdown: Displaying cards for players active at end: {[p+1 for p in self.last_round_active_players]}")
        print(f"Debug Showdown: Current env.hands: {self.env.hands}")

        # <<< THIS LOOP WAS MISSING IN YOUR VERSION >>>
        for pid in range(NUM_PLAYERS):
            label_to_update = self.seat_showdown_card_labels.get(pid)
            if not label_to_update: continue

            # Show cards if the player was active at the end of the round
            should_show_cards = (pid in self.last_round_active_players)

            if should_show_cards:
                hand = self.env.hands.get(pid, []) # Get hand info
                if hand:
                    hand_str = render_hand(hand) # Use utility function
                    label_to_update.config(text=f"Cards: {hand_str}", foreground="darkblue") # Use a distinct color
                    print(f"Debug Showdown: Showing cards for Seat {pid+1}: {hand_str}")
                else:
                    label_to_update.config(text="Cards: N/A", foreground="grey")
                    print(f"Warning: Should show cards for Seat {pid+1}, but no hand found.")
            else:
        ...
        self._update_ui() # Update UI to show cards, final pot, stacks etc.


        # --- Prepare results message for popup ---
        result_message = f"Round Over!\nPot: ${pot_to_distribute:.0f}\n\n"

        if not winners:
            result_message += "No winner determined (Error or unexpected state).\n"
        elif len(eligible_players) == 1: # Won by default
            winner_id = winners[0]
            result_message += f"Seat {winner_id + 1} ({self.seat_configs.get(winner_id, '?').title()}) wins by default.\n"
            result_message += f"Wins: ${win_amount:.2f}\n"
            if winner_id == self.human_player_seat:
                hand_str = render_hand(self.env.hands.get(winner_id, []))
                result_message += f"Your Hand: {hand_str}\n"
        else: # Showdown results
            result_message += "Showdown Results:\n"
            sorted_showdown_players = sorted(eligible_players)
            for pid in sorted_showdown_players:
                hand_str = render_hand(self.env.hands.get(pid, ["?", "?"]))
        ...
        result_message += "\nFinal Stacks (Approximate for display):\n"
        for pid in range(NUM_PLAYERS):
             if self.seat_configs.get(pid) != 'empty':
                 result_message += f"  Seat {pid+1}: ${final_stacks_display.get(pid, 0):.0f}\n"

        # --- Prompt for Next Action (using 'after' to ensure UI update first) ---
        self.root.after(100, lambda: self._prompt_next_round(result_message))

        # <<< REMOVED DUPLICATED CODE BLOCK FROM HERE >>>


    # --- Keep the _prompt_next_round function as defined previously ---
    def _prompt_next_round(self, result_message):
        ...
        if response == 'yes':
            # Check if game should end (only one player with chips) BEFORE starting new round
            active_players_with_chips = [p for p, s in self.env.stacks.items() if s > 0 and self.seat_configs.get(p) != 'empty']
            if len(active_players_with_chips) <= 1:
                 winner_id = active_players_with_chips[0] if active_players_with_chips else -1
                 messagebox.showinfo("Game Over", f"Game Over! Seat {winner_id + 1} wins!\nReconfigure to play again.")
                 self._reconfigure()
            else:
                 self._start_new_round()
        elif response == 'no':
