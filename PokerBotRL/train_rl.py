...
This script implements the training loop for the poker RL agent.
It uses the BestPokerModel from models.py and the TrainFullPokerEnv from envs.py.
Common helper functions (observation encoding, epsilon scheduling, logging, replay buffer)
are imported from utils.py.

The training loop uses an epsilon-greedy strategy and periodically updates a target network.
Checkpoints are saved to disk for later resumption or evaluation.
"""
...
USE_HALF_ENCODING = False  # Set to False to use full encoding.
NUM_PLAYERS = 6
...
def train(agent_checkpoint: torch.nn.Module = None) -> None:
    """
    Trains the RL agent using the BestPokerModel architecture and TrainFullPokerEnv.
    
    The training loop uses an epsilon-greedy policy, experience replay,
    and periodic target network updates. Checkpoints and training metrics are saved.
    ...
    # Instantiate the agent (DQN) and target network.
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        while not done and episode_steps < MAX_EPISODE_STEPS:
            ...
            if env.current_player == env.agent_id:
                ...
                # For non-agent turns, use a default "call" action.
